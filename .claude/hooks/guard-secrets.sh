...
case "$tool" in
  ...
  Bash)
    ...
    #
    # Known false positive, left in deliberately: this matches the *text* of the command, so a
    # heredoc writing documentation that merely names these files (CLAUDE.md §2, AGENTS.md)
    # is refused too. Over-blocking is the right failure direction for a secrets guard, and the
    # workaround is free — write the file with the Write tool instead of a shell heredoc.
