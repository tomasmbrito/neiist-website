...
- DB access → the repository that owns the domain, in `src/lib/db/repositories/`
  (`user` · `team` · `shop` · `event` · `voting`). Queries go through `db_query` from
  `src/lib/db/connection.ts` — **never open your own `Pool` or `Client`.** Business logic
  stays in `src/lib/*` / `src/utils/shop/*`, not in the repository.
- Request validation → the helpers in `src/utils/apiValidationUtils.ts` (`validateId`,
  `validateIstId`, `isValidEmail`, …). **There is no Zod here** — do not introduce it.
- Errors → map through `src/utils/apiErrorUtils.ts`. Never return a raw `pg` error to the
  client.
- UI → compose `@neiist/ui` (`packages/ui/src/components`) before writing a new primitive.
  Button, Modal, Tabs, Card, Field, Avatar, Tooltip, Skeleton, Alert and DateInput all exist.
- Styling → a CSS module under `src/styles/`, mirroring the component path. Not inline styles.
  Colours come from `@neiist/ui/tokens.css`, never a hardcoded hex.
- Pages → under `src/app/[locale]/`, Server Components by default. Any new user-facing string
  goes into **both** `src/i18n/locales/pt.json` and `en.json`.
