...
- The Fenix OAuth callback (`src/app/api/auth/callback/route.ts`): state/CSRF parameter, code
  exchanged server-side, and — critically — an account-linking path that cannot let one
  identity take over another's account by matching on an unverified email.
- `src/proxy.ts` + `src/lib/security/routePermissions.ts`: do they actually protect what they
  claim? Which paths bypass them? Watch the classic trap — a privileged path nested under a
  public prefix (`/shop/manage` under `/shop`) falls through to the public match unless a rule
  claims it explicitly.
- **The proxy is an optimisation, not a boundary.** Every privileged page must also call
  `requireRoles()` from `src/lib/auth.ts`. A page that relies on the proxy alone is a finding;
  8 of the 25 pages currently do not call it — check whether each is genuinely public.
