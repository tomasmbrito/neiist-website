...
- **Check for prior art.** This codebase has a house pattern for most things: repositories in
  `src/lib/db/repositories/`, validation helpers in `src/utils/apiValidationUtils.ts`, error
  mapping in `src/utils/apiErrorUtils.ts`, UI primitives in `packages/ui/src/components`
  (`@neiist/ui`). Reusing the pattern beats inventing a parallel one — and a parallel
  invention is exactly how the previous fork became unmergeable.
- **Do not plan around things that do not exist here.** There is no Zod, no test runner, no
  `withTransaction`, and no `docker/migrations/`. If the plan needs one, that is a
  stop-and-ask, not a step.
- Remember the reset: anything you recall from `archive/fase-1` (workspace, requerimentos,
  team-scoped permissions) is **not in this codebase**. Verify by grep before planning around it.
