...
> **Use `--limit 300`, not 100.** The board passed 100 items once already, and `item-list`
...
- Stale technical assumptions in bodies — especially anything carried over from before the
  2026-09-14 reset. The board and all 49 open issues were cleared on that date, so an item
  referencing a workspace, requerimentos, Zod schemas, `withTransaction` or
  `docker/migrations/` is describing the archived fork (`archive/fase-1`), not this codebase.
