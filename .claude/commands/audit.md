...
- **security-reviewer** — `src/proxy.ts`, `src/lib/auth.ts`, `src/lib/security/**`, all 43
  routes under `src/app/api/**`, `docker/`, `.github/workflows/**`, `scripts/deploy_*.sh`.
  Focus: authn, authz/IDOR, injection, payment tampering, uploads, rate limiting, secrets.
- **data-layer-architect** — `src/lib/db/**`, `docker/schema.sql`, `src/utils/shop/**`,
  `src/instrumentation.ts`. Focus: the absence of transactions and what it costs, race
  conditions, schema constraints and indexes, type/schema drift, N+1 and unbounded queries.
- **quality-reviewer** — `src/components/**`, `src/app/[locale]/**/page.tsx`,
  `src/context/**`, `src/styles/**`, `src/i18n/**`, `packages/ui/**`. Focus: correctness bugs,
  primitives reimplemented instead of taken from `@neiist/ui`, missing or one-sided
  translations, oversized components, duplication, dead code, accessibility, bundle cost.
