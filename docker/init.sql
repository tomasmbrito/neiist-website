...
-- Team names match the 26/27 recruitment branding (docs/ai-workflow/decision-log.md,
-- "Rename departments to the 26/27 recruitment names") — not the club's older Portuguese names.
SELECT  neiist.add_team('Human Resources', 'O trabalho na equipa de HR consiste na criação e partilha de formulários de forma a obter o feedback dos alunos em relação aos eventos organizados pelo NEIIST. É também responsável pelo planeamento das presenças dos membros em bancas e nos eventos bem como pelo recrutamento de novos membros para o núcleo. No final de cada evento é elaborado um relatório para avaliar os resultados e para que os colaboradores saibam o que melhorar em eventos seguintes.');
SELECT  neiist.add_team('External Relations', 'O trabalho na equipa de External Relations consiste em estabelecer e desenvolver relações com empresas, de modo a aproximá-las dos estudantes. Nisto está inserido: reunir com empresas para estabelecer os moldes de uma parceria, angariação de patrocínios para eventos do NEIIST, planeamento de eventos em parceria com empresas, angariação de empresas para os IST Summer Internships, entre outros...');
...
SELECT  neiist.add_team('Marketing', 'O trabalho da equipa de Marketing consiste na coordenação entre a divulgação de todos os eventos organizados pelo NEIIST e de alguns eventos que pedem ao núcleo para divulgar. Os membros desta equipa produzem o texto a seguir para cada evento, divulgando posteriormente pelas redes sociais (ex. Instagram, Facebook e/ou LinkedIn) e pelos grupos (ex. Discord, WhatsApp) de EIC, podendo adaptar-se ao tipo de evento e ao público alvo. E agora são ainda responsáveis por fazer os famosos reels do NEIIST!');
SELECT  neiist.add_team('Photography', 'O trabalho da equipa de Photography consiste na cobertura fotográfica e/ou videográfica de eventos organizados pelo NEIIST de modo a expandir a nossa galeria e a mostrar a todos os interessados o trabalho do núcleo. Os membros desta equipa fotografam e/ou filmam os eventos e depois editam o material para ficar pronto para publicação.');
SELECT  neiist.add_team('Logistics', 'A equipa de Logistics é algo diferente do trabalho nas restantes equipas, pode variar bastante de evento para evento, mas inclui sempre tratar da logística, falar com possíveis oradores e/ou outros intervenientes na organização do evento (talvez até falar com possíveis patrocinadores, se for esse o caso) e fazer a ponte com as outras equipas do NEIIST envolvidas no evento.');
SELECT  neiist.add_team('Design', 'O trabalho da equipa de Design consiste na criação de cartazes, banners, panfletos e outros materiais visuais para ajudar à divulgação de eventos organizados pelo NEIIST, para garantir que estes chegam ao maior número possível de alunos. Os membros da equipa produzem o material pedido e recebem feedback da equipa, antes de o enviar para os organizadores do evento que pedem as alterações necessárias, se for esse o caso.');
...
SELECT neiist.add_valid_department_role('Human Resources', 'Coordenador', 'coordinator');
SELECT neiist.add_valid_department_role('Human Resources', 'Membro', 'member');

SELECT neiist.add_valid_department_role('External Relations', 'Coordenador', 'coordinator');
SELECT neiist.add_valid_department_role('External Relations', 'Membro', 'member');
...
SELECT neiist.add_valid_department_role('Marketing', 'Coordenador', 'coordinator');
SELECT neiist.add_valid_department_role('Marketing', 'Membro', 'member');

SELECT neiist.add_valid_department_role('Photography', 'Coordenador', 'coordinator');
SELECT neiist.add_valid_department_role('Photography', 'Membro', 'member');

SELECT neiist.add_valid_department_role('Logistics', 'Coordenador', 'coordinator');
SELECT neiist.add_valid_department_role('Logistics', 'Membro', 'member');

SELECT neiist.add_valid_department_role('Design', 'Coordenador', 'coordinator');
SELECT neiist.add_valid_department_role('Design', 'Membro', 'member');
