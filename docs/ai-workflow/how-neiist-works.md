...
**Written 2026-08-25 from what Tomás told me directly.** Everything here is domain knowledge
that cannot be derived from the code. It is about how the núcleo operates, not about any
particular codebase, which is why it survived the 2026-09-14 reset onto upstream v3.0.0 when
almost everything else in this directory did not.

**Read this before designing anything that touches how the association runs.** Where it
disagrees with the code, this file is right about NEIIST — the code simply has not caught up.

Two caveats after the reset:

- **The members-only workspace it describes does not exist in this codebase.** It was built in
  the previous phase and is now archived at tag `archive/fase-1`. Read the sections below as a
  description of NEIIST's process, not of shipped features.
- **Issue numbers here (`#131`, `#137`, …) refer to the old board**, which was cleared on
  2026-09-14. They are historical pointers, not open work.
