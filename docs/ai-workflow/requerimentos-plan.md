...
> ⚠️ **Pre-reset document — product context only.** On 2026-09-14 this fork was reset onto
> upstream v3.0.0, discarding the codebase this document was written against. The *product*
> reasoning below still holds and is why the file was kept. Every reference to implementation —
> `src/components/workspace/`, `internal_events`, Zod schemas, `withTransaction`,
> `docker/migrations/`, board issue numbers — describes the archived fork (tag `archive/fase-1`),
> **not this codebase**. Verify by grep before planning around anything here.
